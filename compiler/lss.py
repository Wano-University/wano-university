...
reserved = {"if": "IF", "then": "THEN", "else": "ELSE", "mobility": "MOBILITY"}
...
def mobility_resources_to_text(resources):
    ...
    return "\n".join(
        f"MobilityResource {r['id']}: {r['type']} — Identifier {r['identifier']}, Location {r['location']}, Status {r['status']}."
        for r in resources
    )
...
def sensors_to_text(sensors):
    if not sensors:
        return "No sensors"
    return "\n".join(
        f"Sensor {s['id']}: {s['type']} — Space {s['space']}, Floor {s['floor']} "
        for s in sensors
    )
...
def p_instruction_rent_resource(p):
    ...
    response = requests.post(
        f"{API_URL}/api/reservations/", json=payload, headers=get_headers()
    )
...
def p_instruction_get_reservations(p):
    ...
    response = requests.get(f"{API_URL}/api/reservations/", headers=get_headers())
...
def p_instruction_get_reservations_by_user(p):
    ...
    response = requests.get(
        f"{API_URL}/api/reservations/user/{p[3]}", headers=get_headers()
    )
    if response.status_code == 200:
        p[0] = reservations_to_text(response.json())
...
def p_instruction_update_reservation(p):
    ...
    status = {"status": p[4].upper()}
    response = requests.patch(
        f"{API_URL}/api/reservations/{p[3]}", json=status, headers=get_headers()
    )
    if response.status_code == 200:
...
def p_instruction_register_user(p):
    ...
    response = requests.post(
        f"{API_URL}/api/users/register", json=p[0], headers=get_headers()
    )
...
def p_instruction_register_resource(p):
    ...
    p[0] = resource
    response = requests.post(
        f"{API_URL}/api/resources/", json=resource, headers=get_headers()
    )
    if response.status_code == 201:
...
def p_instruction_get_resources(p):
    ...
    response = requests.get(f"{API_URL}/api/resources/", headers=get_headers())
...
def p_instruction_get_resources_by_type(p):
    ...
    response = requests.get(
        f"{API_URL}/api/resources/type/{resource_type}", headers=get_headers()
    )
...
def p_instruction_register_sensor(p):
    ...
    response = requests.post(
        f"{API_URL}/api/sensors/", json=payload, headers=get_headers()
    )
...
def p_instruction_get_sensors(p):
    ...
    response = requests.get(f"{API_URL}/api/sensors/", headers=get_headers())
...
def p_instruction_get_sensors_by_type(p):
    ...
    response = requests.get(f"{API_URL}/api/sensors/type/{p[3]}", headers=get_headers())
...
def p_instruction_get_sensors_by_floor(p):
    ...
    response = requests.get(
        f"{API_URL}/api/sensors/floor/{p[3]}", headers=get_headers()
    )
...
def p_instruction_get_mobility_resources(p):
    ...
    response = requests.get(f"{API_URL}/api/mobilityResources/", headers=get_headers())
...
def p_instruction_get_mobility_resources_by_type(p):
    ...
    response = requests.get(
        f"{API_URL}/api/mobilityResources/type/{p[3]}", headers=get_headers()
    )
...
def p_instruction_register_mobility_resource(p):
    ...
    response = requests.post(
        f"{API_URL}/api/mobilityResources/", json=payload, headers=get_headers()
    )
    ...
    if response.status_code == 201:
        ...
        p[0] = data.get(
            "error", data.get("message", "Failed to register mobility resource")
        )
...
def p_instruction_update_mobility_status(p):
    ...
    response = requests.patch(
        f"{API_URL}/api/mobilityResources/{p[3]}", json=payload, headers=get_headers()
    )
    ...
    if response.status_code == 200:
        ...
        data = response.json()
        p[0] = data.get(
            "error", data.get("message", "Failed to update mobility resource")
        )
