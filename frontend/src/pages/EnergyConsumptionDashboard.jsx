...
export default function EnergyConsumptionDashboard() {
  ...
  const totalPower = data?.totalPower ?? "0 W"
  ...
  const handleUpdateLimits = async () => {
    try {
      await updateSensorLimits(selectedSensorId, {
  ...
  const handleExport = async () => {
    setIsExporting(true)
    try {
      const blob = await getEnergyReport()
      const url = window.URL.createObjectURL(blob)
      const a = document.createElement("a")
      a.href = url
      a.download = `energy_report_${new Date().toISOString().slice(0, 7)}.csv`
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        padding: 10,
        displayColors: false,
        callbacks: { label: (context) => `${context.parsed.y} W` }
      }
    },
    scales: {
      x: { grid: { display: false, drawBorder: false }, ticks: { color: 'hsl(var(--muted-foreground))', font: { size: 11 } } },
      y: { grid: { color: 'hsl(var(--border))', borderDash: [3, 3] }, ticks: { color: 'hsl(var(--muted-foreground))', font: { size: 11 }, callback: (value) => `${value}W` }, border: { display: false } }
    }
  };
            ...
            {isLoading
              ...
              : currentSensors.length > 0
                ? currentSensors.map(sensor => {
                  ...
                  const isOverLimit = sensor.upperLimit !== null && sensor.val > sensor.upperLimit
                  return (
                    <Card
                      key={sensor.id}
                    ...
                    >
                      <Zap className={`w-5 h-5 mb-4 ${isOverLimit ? "text-destructive" : "text-primary"}`} />
                      <p className="text-2xl font-black text-foreground">{sensor.val} W</p>
